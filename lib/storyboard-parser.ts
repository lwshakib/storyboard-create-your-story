 ...
 * Using off-screen rendering for accurate coordinate and style extraction.
 */
export async function htmlToStructuredSlide(htmlSlide: HtmlSlide): Promise<Slide> {
  ...
  if (typeof window === 'undefined') {
    return { id: htmlSlide.id, elements: [], bgColor: '#ffffff', layout: 'free' };
  }
  ...
  const findBackground = (node: HTMLElement): { color?: string, image?: string } => {
    ...
    if (bg && bg !== 'none') {
        if (bg.includes('url')) {
            ...
            const colorMatch = bg.match(/#(?:[0-9a-fA-F]{3}){1,2}|rgb\(.*?\)|rgba\(.*?\)/);
            if (colorMatch) color = colorToHex(colorMatch[0]);
        }
    }
    const isMeaningful = color && color !== 'rgba(0, 0, 0, 0)' && color !== 'transparent' && color !== '#00000000';
    if (isMeaningful || image) return { color: isMeaningful ? color : undefined, image };
  ...
  const traverse = (node: Element) => {
    ...
    const textTags = ['H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'P', 'SPAN', 'DIV', 'LI', 'B', 'I', 'STRONG', 'EM'];
    const hasText = Array.from(node.childNodes).some(n => n.nodeType === 3 && n.textContent?.trim().length);

    if (isImage) {
        elements.push({ id: Math.random().toString(36).substr(2, 9), type: 'image', src: (node as HTMLImageElement).src, x, y, width: width || 400, height: height || 300, objectFit: (style.objectFit as any) || 'cover' });
    } else if (textTags.includes(node.tagName) && hasText) {
        const content = (node as HTMLElement).innerText.trim();
        if (content) {
            elements.push({ id: Math.random().toString(36).substr(2, 9), type: 'text', content, x, y, width: width || rect.width, height: height || rect.height, fontSize: parseInt(style.fontSize) || 24, color: colorToHex(style.color || '#000000'), fontWeight: style.fontWeight || 'normal', textAlign: (style.textAlign as any) || 'left', fontFamily: style.fontFamily?.replace(/['"]/g, '') || 'Inter, sans-serif' });
        }
    } else if (node.tagName === 'DIV' || node.tagName === 'SPAN') {
        const bg = colorToHex(style.backgroundColor);
        const hasBg = bg && bg !== 'rgba(0, 0, 0, 0)' && bg !== 'transparent' && bg !== '#00000000';
