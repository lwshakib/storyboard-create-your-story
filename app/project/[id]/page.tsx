...
function EditorContent() {
    ...
    (serverSlides: HtmlSlide[], finishedSkeletonId?: number) => {
      setStreamingSlides((prev) => {
        ...
        const pendingSkeletons = prev.filter(
          (s) =>
            typeof s.id === "number" && s.id < 0 && s.id !== finishedSkeletonId
        )
  ...
  const fetchCredits = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        Promise.resolve().then(() => setCredits(data.credits))
      }
    } catch (err) {
  ...
  const generateOutline = useCallback(
    async (p: string) => {
      ...
      try {
        ...
        Promise.resolve().then(() => setIsGeneratingOutline(false))
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          {isGeneratingOutline && (
            <Button
              ...
              onClick={() => {
                if (outlineAbortControllerRef.current) {
  ...
  if (!project) {
    return (
      <div className="flex h-screen w-full flex-col items-center justify-center gap-6 bg-[#F8F9FB] text-center dark:bg-[#0A0A0B]">
        <div className="bg-destructive/5 text-destructive rounded-none p-4 text-xs font-bold tracking-tight">
          Project not found.
        </div>
        ...
        <div className="flex flex-col items-center gap-4">
          {prompt && (
            <Button
              className="h-12 rounded-none px-10 text-[11px] font-black tracking-widest transition-all"
              onClick={() => {
          ...
          <Link
            href="/home"
            className="text-muted-foreground text-[10px] font-black tracking-widest transition-all hover:text-black hover:underline dark:hover:text-white"
          >
            Go back home
          </Link>
        </div>
  ...
  return (
    <ProjectView
      ...
      error={error}
      initialPrompt={lastPrompt || prompt}
      onRetryOutline={() => {
        if (lastPrompt) {
          Promise.resolve().then(() => setError(false))
          generateOutline(lastPrompt)
        }
      }}
      onCancelOutline={() => {
        if (outlineAbortControllerRef.current) {
