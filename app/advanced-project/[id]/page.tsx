...
import AdvancedEditorPage from "../../advanced-editor/page"

export default function AdvancedProjectViewPage() {
  const { id } = useParams()
  ...
  React.useEffect(() => {
    const fetchProject = async () => {
      try {
        const res = await fetch(`/api/advanced-projects/${id}`)
        if (res.ok) {
        ...
        console.error("Failed to fetch advanced project", err)
        setError(true)
      } finally {
        setLoading(false)
      }
  ...
  if (loading) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center gap-4 bg-[#F8F9FB] dark:bg-[#0A0A0B]">
        ...
        <p className="text-sm font-medium text-muted-foreground animate-pulse">Opening your advanced storyboard...</p>
      </div>
    )
  }
    ...
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center gap-4 bg-[#F8F9FB] dark:bg-[#0A0A0B]">
        <div className="p-4 rounded-none bg-destructive/5 text-destructive text-sm font-bold uppercase tracking-tight">
          Advanced Project not found or failed to load.
        </div>
    ...
    return (
        <div className="h-screen w-full flex flex-col items-center justify-center gap-6 bg-[#F8F9FB] dark:bg-[#0A0A0B] text-center">
          ...
          <div className="space-y-2">
            <h1 className="text-xl font-extrabold tracking-tight">This advanced storyboard is in the trash</h1>
  ...
  // We need to pass the existing project data to AdvancedEditorPage
  // Let's modify AdvancedEditorPage to accept initialData
