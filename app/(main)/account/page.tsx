          ...
          <div className="grid gap-6">
            ...
            <Card>
              ...
              <CardFooter className="border-t px-6 py-4">
                <Button onClick={handleUpdateName} disabled={isUpdatingName || name === session.user.name}>
        ...
        <div className="space-y-4">
            {sessions?.map((session: any) => (
                <div key={session.id} className="flex items-center justify-between p-3 border rounded-lg">
                    ...
                    {session.token !== currentSession?.session?.token && (
                        <Button variant="ghost" size="sm" onClick={() => handleRevoke(session.token)}>Revoke</Button>
                    )}
                </div>
            ))}
            {sessions?.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">No other active sessions.</p>}
        </div>
    )
}
